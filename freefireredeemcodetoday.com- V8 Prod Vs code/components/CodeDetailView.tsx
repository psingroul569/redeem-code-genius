...
import { ArrowLeft, Copy, Check, Zap, ThumbsUp, Loader2, Send, History } from 'lucide-react';
...
export const CodeDetailView: React.FC<CodeDetailViewProps> = ({ code, setView, lastSyncTime }) => {
  ...
  useEffect(() => {
    const loadComments = async () => {
      ...
      try {
        ...
        // Combine static mock comments with local visitor comments
        const combined = [
          ...
          <div className="grid md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/5">
              <div className="md:col-span-2 p-8 md:p-12 bg-black">
                ...
                <div className="space-y-12">
                    {[
                      { title: "Official Garena Site", desc: <>Visit <a href="https://reward.ff.garena.com" className="text-cyber-success font-bold" target="_blank">reward.ff.garena.com</a></> },
              ...
              <div className="p-8 bg-[#030303] flex flex-col h-[750px]">
                 <div className="flex items-center justify-between mb-8">
                    <h3 className="text-xs font-bold text-white uppercase tracking-widest flex items-center gap-2">
                        ...
                        Community Log (Node: {code.server})
                    </h3>
                 </div>
                          ...
                          <div key={comment.id} className={`bg-white/5 p-5 rounded-2xl border transition-all animate-fade-in ${comment.isVisitor ? 'border-cyber-success/30' : 'border-white/5'}`}>
                              <div className="flex items-center justify-between mb-3">
                                  <div className="flex items-center gap-3">
                                      <div className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-[10px] text-white bg-white/5">
                                      ...
                                      <div>
                                         ...
                                         <div className="text-[7px] text-white/20 uppercase tracking-widest">
                                           {comment.isVisitor ? 'Visitor' : 'Community User'}
                                         </div>
                                      </div>
                                  </div>
                 ...
                 <form onSubmit={handlePostComment} className="mt-6 pt-6 border-t border-white/10 relative">
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl py-4 pl-4 pr-12 text-xs text-white focus:border-cyber-success outline-none transition-all" 
                    />
