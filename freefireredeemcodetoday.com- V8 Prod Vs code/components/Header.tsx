...
import { Radio, Zap, Globe, Activity, ShieldCheck } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 bg-black/90 backdrop-blur-xl pt-4 pb-4 z-50 border-b border-white/5 transition-all shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between gap-4">
        ...
        {/* Professional Brand Assembly */}
        <div 
          className="flex items-center gap-3 md:gap-5 group cursor-pointer min-w-0" 
          onClick={() => setView('home')}
        >
          {/* Custom Shield-Bolt Icon (Stays on Mobile) */}
          ...
          {/* Brand Text - Hidden on Mobile, Flex on Desktop */}
          <div className="hidden md:flex flex-col min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[9px] text-cyber-muted tracking-[0.25em] font-tech uppercase font-bold truncate">
                  HOURLY VERIFICATION ENGINE
                </span>
        ...
        <div className="flex items-center gap-3 md:gap-6 flex-shrink-0">
           {/* Navigation Core (Stays on Mobile) */}
           <nav className="flex items-center space-x-2 md:space-x-4">
              <button 
                onClick={() => setView('home')}
                className={`px-4 md:px-6 py-2.5 rounded-xl text-[10px] md:text-xs font-tech font-bold tracking-widest transition-all border shadow-sm ${
                  currentView === 'home' 
              ...
              <button 
                ...
                className={`px-4 md:px-6 py-2.5 rounded-xl text-[10px] md:text-xs font-tech font-bold tracking-widest transition-all border shadow-sm ${
                  ['content', 'article'].includes(currentView)
                  ? 'bg-white text-black border-white shadow-white/10' 
                  : 'text-gray-400 hover:text-white border-white/5 hover:bg-white/5'
                }`}
              >
