...
const CodeDetailView = React.lazy(() => import('./components/CodeDetailView').then(m => ({ default: m.CodeDetailView })));

const REGIONS = ['GLOBAL', 'INDIA', 'BRAZIL', 'INDONESIA', 'EUROPE'];
const REGION_OFFSETS: Record<string, number> = {
  'GLOBAL': 0, 'INDIA': 3, 'BRAZIL': 6, 'INDONESIA': 9, 'EUROPE': 12    
};
...
function App() {
  ...
  const performAtomicSync = useCallback(async (region: string, force = false) => {
            ...
            <section className="relative w-full pt-8 pb-2 px-4 flex flex-col items-center">
                <div className="max-w-7xl mx-auto text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full mb-8">
                       ...
                       <span className="text-[10px] font-tech font-bold text-white/60 uppercase tracking-widest">Global Cloud Consensus Engine Active</span>
                    </div>
                    ...
                    <div className="flex flex-row items-center gap-3 w-full md:w-auto">
                      ...
                      <div className={`flex-1 md:flex-initial px-6 py-3 rounded-2xl border transition-all flex items-center justify-center gap-5 ${isOverdue ? 'bg-red-500/10 border-red-500/20' : 'bg-[#081510] border-cyber-success/10'}`}>
                          ...
                          <div className="flex flex-col">
                              <span className="text-[8px] text-white/30 font-tech uppercase tracking-widest">{isOverdue ? 'Syncing Missing Block' : 'Next Broadcast'}</span>
            ...
            <section id="codes" className="max-w-7xl mx-auto px-4 md:px-8 py-12 min-h-[400px]">
                {isSyncing && displayCodes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 text-center">
                        ...
                        <p className="font-tech text-white/40 uppercase tracking-widest text-xs">Authenticating with {activeRegion} Cloud Nodes...</p>
                    </div>
                ) : (
                ...
                {!isSyncing && displayCodes.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <p className="font-tech text-white/40 uppercase tracking-widest text-xs">No unique codes found for this region yet.</p>
                    <button onClick={() => performAtomicSync(activeRegion, true)} className="mt-4 px-6 py-2 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all">Retry Regional Sync</button>
                  </div>
                )}
            </section>
            <AuthorityHub />
